...
export const scaffoldBlocks = (ssConfig: ProjectConfig) => {
  ...
  const fragmentTemplatePath = path.join(
    libraryDir,
    `src/server/templates/elements.template`
  )
  if (!fs.existsSync(fragmentTemplatePath)) {
    throw new Error(`Template ${fragmentTemplatePath} does not exist`)
  }
  const fragmentTemplateContents = fs.readFileSync(fragmentTemplatePath, {
    encoding: `utf8`,
  })
  ...
  const componentTemplateContents = fs.readFileSync(componentTemplatePath, {
    encoding: `utf8`,
  })
  ...
  api.queryUncached(query, variables).then(result => {
    ...
    if (!baseElement) {
      throw new Error(
        `No BaseElement type could be found. Perhaps you're using a custom type name?`
      )
    }
    ...
    const baseFieldsPath = path.relative(
      absElementalFragmentsPath,
    ...
    result.generateFragments.forEach((result: FragmentResult) => {
      ...
      if (!result.fragment) {
        console.log(
          `${result.type} has no unique fields. Skipping fragment generation.`
        )
        return
      }
    ...
    const absElementsPath = path.join(
      absElementalFragmentsPath,
      `_elements.graphql`
    )
    ...
    if (needsRegeneration) {
      ...
      console.log(
        `_elements.graphql file has been customised. Skipping regeneration.`
      )
    }
  })
}
